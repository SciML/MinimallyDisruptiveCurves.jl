...
@safetestset "Sparse Initialization Utilities" begin
    ...
    @testset "sparse_init_dir" begin
        ...
        @test norm(v) ≈ 1.0 atol = 1.0e-6  # Must be normalized
        @test val ≈ 0.1 rtol = 1.0e-2       # Rayleigh quotient should approach the smallest eigenvalue
        ...
        @test norm(v1) ≈ 1.0 atol = 1.0e-6
        @test abs(dot(v1, v2)) < 1.0e-5   # Must be orthogonal to v1
    end
